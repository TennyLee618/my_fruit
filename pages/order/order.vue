	...
	<view class="container">
		...
		<checkbox-group @change="buyChange">
			<view class="tui-cart-cell  tui-mtop" v-for="(item, index) in orderObj" :key="index">
				...
				<tui-swipe-action :actions="actions" @click="handlerButton(itemTwo.id, itemTwo.goodsId)" :params="item" v-for="(itemTwo, indexs) in item.list" :key="indexs">
...
<script>
//请求方式
import { listing, publicing, listing2, publicing2 } from '../../api/api.js';
//请求地址
import { getCart, postDelOrder, postUpOrder, getClient, postSubmitOrder, loginis } from '../../api/request.js';
var { log } = console;
